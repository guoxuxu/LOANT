...
class Trainer(object):
    ...
    def run(self):
        ...
        if self.options['model_name'] in ['mtl', 'mtl_lo', 'mtl_maml', 'ANT', 'LOANT']:
            from .mtl_train_batch import train_batch
        elif self.options['model_name'] in ['bert']:
        ...
        for epoch in range(0, self.options['training_epochs']):
            ...
            for i, batch in enumerate(train_loader):
                ...
                if self.options['model_name'] in ['ANT', 'LOANT']:
                ...
                with torch.cuda.amp.autocast():
                    train_loss, batch_forward_time, train_log_line = train_batch(scaler, model, optimizer, batch, self.options, log=log, ad_weight=ad_weight, ad=ad)
                ...
                if (start_step + i) % self.options['log_every'] == 0:
                    ...
                    if self.options['model_name'] in ['mtl_lo', 'mtl_maml', 'LOANT', 'ant_maml']:
